        ...
        for j in range(N):
            ...
            for snr in [5, 30]:
                ...
                results = least_squares(
                    self.system, x0, args=(mean_phase_diff, mean_AoA)
                )
                ...
                if save_all:
                    ...
                    eta_rel_error.append(
                        abs((self.eta - np.mean(results.x[2])) / self.eta)
                    )
                    v_abs_error.append(abs(self.v_rx - np.mean(results.x[1])))
                    v_rel_error.append(
                        abs((self.v_rx - np.mean(results.x[1])) / self.v_rx)
                    )
...
if __name__ == "__main__":
    ...
    for fc in [5, 28]:
        ...
        print(
            "average fd estimate relative error, with fc= "
            + str(fc)
            + str(np.mean(inter_error, axis=0))
            + "\n"
        )
        print(
            "median fd estimate relative error, with fc= "
            + str(fc)
            + str(np.median(inter_error, axis=0))
            + "\n"
        )
