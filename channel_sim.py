...
class channel_sim:
    ...
    def __init__(
        ...
        self.std_w = fo_max / (
            3 * np.sqrt(n**3)
        )  # --->  std for the fo random walk s.t. its max drift in 1 ms is fo_max #  fo_max/(6*np.pi*self.T*(n+1))# std for independent samples of the cfo s.t. its max drift in 1 ms is fo_max
        a = 0
        ...
        for i, d in enumerate(delays.astype(int)):
            if self.l == 0.005:
                up_cir[d * self.us] = up_cir[d * self.us] + self.paths["gain"][
                    i
                ] * np.exp(1j * 2 * np.pi * self.T * self.k * self.paths["phase"][i])
            cir[d] = cir[d] + self.paths["gain"][i] * np.exp(
                1j * 2 * np.pi * self.T * self.k * self.paths["phase"][i]
            )
    ...
    def simulation(
        ...
        for j in range(N):
            ...
            if save_all:
                eta_abs_error.append(
                    abs(np.rad2deg(self.eta) - np.rad2deg(np.mean(results.x[2])))
                )
        ...
        if save:
            if self.l == 0.005:
                ...
                if save_time:
                    np.save(
                        path + "time/nlsTime_fc5_ns" + str(self.n_static) + ".npy",
                        nls_time,
                    )
            elif self.l == 0.0107:
                np.save(
...
def varying_static_paths():
    ...
    for n_static in [8]:
        for l in [0.0107, 0.06]:
            ...
            npath_error, nls_time = ch_sim.simulation(
                x_max=s,
                y_max=s,
                N=10000,
                interval=i,
            ...
            np.save(
                "cir_estimation_sim/data/varying_npath/tot_5_fd_error_fc%s.npy"
                % (int(3e8 / l * 1e-9)),
                npath_error,
            )
            np.save(
                "cir_estimation_sim/data/varying_npath/tot_5_nls_time_fc%s.npy"
                % (int(3e8 / l * 1e-9)),
                nls_time,
            )
            print(
                "average fd estimate relative error: "
                + str(np.mean(npath_error, axis=0))
                + "\n"
            )
            print(
                "median fd estimate relative error: "
                + str(np.median(npath_error, axis=0))
                + "\n"
            )
    ...
    for snr in [5]:
        for l in [0.0107, 0.06]:
            for a in [5]:
                ...
                i = int(interval * 1e-3 / ch_sim.T)
                fd_error = ch_sim.simulation(
                    x_max=s,
                    y_max=s,
                    N=10000,
                    interval=i,
                    path="cir_estimation_sim/data/varying_snr/aoa" + str(a) + "/",
                    save=False,
                )
                print(
                    "average fd estimate relative error: "
                    + str(np.mean(fd_error))
                    + "\n"
                )
                print(
                    "median fd estimate relative error: "
                    + str(np.median(fd_error))
                    + "\n"
                )
    ...
    for interval in [2, 4, 8, 16, 32]:
        for l in [0.005, 0.0107]:
            ...
            fd_error = ch_sim.simulation(
                ...
                N=10,
                interval=i,
                path="data/varying_interval/",
                save=False,
            )
            print(
                "average fd estimate relative error: " + str(np.mean(fd_error)) + "\n"
            )
            print(
                "median fd estimate relative error: " + str(np.median(fd_error)) + "\n"
            )
...
def varying_T():
    for f in [28]:
        ...
        for ns in [2, 4, 6]:
            ...
            for t in times:
                ...
                if f == 28:
                    ### fc = 28 GHz ###
                    ch_sim = channel_sim(
                        vmax=10, SNR=20, l=0.0107, AoAstd=0, n_static=ns, T=t
                    )
                error = ch_sim.simulation(
                    x_max=20,
                    y_max=20,
                    path=path + "fc_" + str(f) + "/" + str(ns) + "_static/",
                    save=False,
                    N=10000,
                    interval=200,
                )
        ...
        np.save(
            path + "fc_" + str(f) + "_" + str(ns) + "_static_snr0_f_d_error_nb.npy",
            tot_f_d_error,
        )
...
if __name__ == "__main__":
    ...
    fd_error = ch_sim.simulation(
        ...
        N=100,
        interval=i,
        path="cir_estimation_sim/data/varying_snr/aoa3/",
        save=False,
    )
