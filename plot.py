...
def plot_fd():
    ...
    for fc in fcs:
        ...
        plot_boxplot(
            "cir_estimation_sim/plot" + subdir + "/",
            errors,
            "No. of paths",
            "fD relative error",
            np.array(labels),
            "varying npath AoA=5°, fc=%s GHz" % (fc),
            "var_npathfc_%s" % (fc),
        )
...
def plot_speed():
    ...
    for fc in fcs:
        ...
        if fc == 60:
            ...
            for i in range(3):
                print(
                    "\n\naverage speed estimate relative error, AoA std=%s° " % (aoa[i])
                    + str(np.mean(data[:, :, i], axis=0))
                    + "\n"
                )
                print(
                    "median speed estimate relative error, AoA std=%s° " % (aoa[i])
                    + str(np.median(data[:, :, i], axis=0))
                    + "\n"
                )
                plot_boxplot(
                    "cir_estimation_sim/plot/",
                    data[:, 1:, i],
                    "SNR [dB]",
                    "speed relative error",
                    np.array(labels),
                    "varying snr AoA=%s°, fc=%s GHz" % (aoa[i], fc),
                    "var_snr_relspeed_fc_%sa_%s" % (fc, aoa[i]),
                )
        else:
            aoa = [5]
            i = 0
...
def plot_eta():
    ...
    for fc in fcs:
        ...
        if fc == 60:
            ...
            for i in range(3):
                print(
                    "\n\naverage eta estimate relative error, AoA std=%s° " % (aoa[i])
                    + str(np.mean(data[:, :, i], axis=0))
                    + "\n"
                )
                print(
                    "median eta estimate relative error, AoA std=%s° " % (aoa[i])
                    + str(np.median(data[:, :, i], axis=0))
                    + "\n"
                )
                plot_boxplot(
                    ...
                    np.array(labels),
                    "varying snr AoA=%s°, fc=%s GHz" % (aoa[i], fc),
                    "rel_var_snr_eta_fc_%sa_%s" % (fc, aoa[i]),
                )
        else:
            aoa = [5]
            i = 0
...
def plot_interval():
    ...
    for fc in fcs:
        ...
        plot_boxplot(
            ...
            "fD relative error",
            np.array(labels),
            "varying interval AoA=5°, fc=%s GHz" % (fc),
            "var_interfc_%s" % (fc),
        )
...
def plot_snr():
    ...
    for fc in fcs:
        ...
        if fc != 28:
            ...
            for i in range(3):
                print(
                    "\n\naverage fd estimate relative error, AoA std=%s° " % (aoa[i])
                    + str(np.mean(data[:, :, i], axis=0))
                    + "\n"
                )
                print(
                    "median fd estimate relative error, AoA std=%s° " % (aoa[i])
                    + str(np.median(data[:, :, i], axis=0))
                    + "\n"
                )
                plot_boxplot(
