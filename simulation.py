...
def check_system():
    for i in range(10000):
        ...
        c_phases = check_phases(eta_n[0], f_d_n[0], v_n[0], f_off_n[0], zetas)
        for j in range(4):
...
for z_std in np.deg2rad(zeta_std):
    ...
    for phase_std,snr in zip(p_std,SNR):
        ...
        for i in range(N):
            ...
            eta_error.append(eta-eta_n[1])
            f_d_error.append(f_d-f_d_n[0])
            v_error.append(v-v_n[1])
            f_off_error.append(f_off-f_off_n)
