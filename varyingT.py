...
class channel_sim:
    ...
    def __init__(
        ...
        self.paths = np.zeros(
            (n_static + 2, 4)
        )  # [delay, phase, attenuations, AoA] for each path [LoS,t,s1,...,sn_static]
        self.paths = {
        ...
        self.std_w = fo_max / (
            n
        )  # fo_max/(3*np.sqrt(n**3)) # std for the fo random walk s.t. its max drift in 1 ms is fo_max
        a = 0
        ...
        for t in range(int((n + 1) / 2)):
            if t == 0:
                ...
                h[start + t] = (
                    ...
                    * (
                        (
                            np.sin(np.pi * t / T * (1 - beta))
                            + (4 * beta * t / T * np.cos(np.pi * t / T * (1 + beta)))
                        )
                        / (np.pi * t / T * (1 - (4 * beta * t / T) ** 2))
                    )
                )
    ...
    def compute_cir(self, init, T=0, plot=False):
        """
        ...
        for i, d in enumerate(delays.astype(int)):
            if self.l == 0.005:
                up_cir[d * self.us] = up_cir[d * self.us] + self.paths["gain"][
                    i
                ] * np.exp(1j * 2 * np.pi * T * self.k * self.paths["phase"][i])
            cir[d] = cir[d] + self.paths["gain"][i] * np.exp(
                1j * 2 * np.pi * T * self.k * self.paths["phase"][i]
            )
    ...
    def gen_noise(self, len):
        ...
        ) + 1j * np.random.normal(0, noise_std / np.sqrt(2), size=len)
        return noise
    ...
    def sampling(self, up_rx_signal, plot=False):
        ...
        xcorr = np.correlate(up_rx_signal, rcos_up_Ga)[
            0 : 256 * self.us
        ]  # check just for the first peak
        # plt.plot(xcorr.real)
        # plt.plot(abs(xcorr))
        # plt.show()
    ...
    def add_cfo(self, signal, T=0):
        ...
        return signal * np.exp(1j * 2 * np.pi * self.f_off * self.k * T)
    ...
    def estimate_ofdm_CIR(self, Y, plot=False):
        ...
        if plot:
            plt.stem(
                np.abs(g) / max(abs(g)), linefmt="r", markerfmt="rD", label="estimate"
            )
            # plt.stem(np.linspace(0,256,16*256),abs(self.up_cir)/max(abs(self.up_cir)), linefmt='g', markerfmt='gD', label='real upsampled')
            plt.stem(np.abs(self.cir) / max(abs(self.cir)), label="real sampled")
            plt.legend()
    ...
    def solve_system(self, phases, zetas, T=0):
        """
        ...
        if T == 0:
            T = self.T
        alpha = phases[2] * (np.cos(zetas[1]) - 1) - (
            phases[1] * (np.cos(zetas[2]) - 1)
        )
        ...
        A = (np.sin(zetas[1]) * (1 - np.cos(zetas[2]))) + (
            np.sin(zetas[2]) * (np.cos(zetas[1]) - 1)
        )
        if not (beta) / A > 0:
            eta = eta + np.pi
        if eta < 0:
        ...
        f_d = (
            phases[0]
            - (
                (phases[1] * (np.cos(zetas[0] - eta) - np.cos(eta)))
                / (np.cos(zetas[1] - eta) - np.cos(eta))
            )
        ) / (2 * np.pi * T)
    ...
    def system(self, x, phases, n_zetas, T=0):
        """
        ...
        results.append(
            ...
            - (
                ...
                * np.pi
                * T
                * (x[0] + (x[1] / self.l * (np.cos(n_zetas[0] - x[2]) - np.cos(x[2]))))
            )
        )
        ...
        for i in range(1, len(phases)):
            results.append(
                ...
                - (
                    ...
                    * np.pi
                    * T
                    * (x[1] / self.l * (np.cos(n_zetas[i] - x[2]) - np.cos(x[2])))
                )
            )
    ...
    def plot_mae(self, dataa, path, legend, times):
        ...
        for mean, std in zip(m, s):
            low = mean - (3 * std)
            low[low < 0] = 0
            plt.fill_between(times, low, mean + (3 * std), alpha=0.3)
        plt.grid()
        plt.xlabel("time (ms)")
    ...
    def simulation(self, x_max, y_max, N, interval, relative=True):
        """
        ...
        for j in range(N):
            ...
            for t in np.arange(0.18, 0.72, 0.02):
                t = t * 1e-3
                phase_diff = []
                self.compute_cir(init=True, T=t, plot=False)
                if self.l == 0.005:
                ...
                ### add cfo ###
                h = self.add_po(self.add_cfo(h, T=t))
                self.get_phases(h)
                for p in range(1, len(self.phases[:, 1])):
                    self.phases[p, 1] = self.phases[p, 1] - self.phases[0, 1]
                AoA = [
                    self.paths["AoA"][1:]
                    + np.random.normal(0, self.AoAstd, self.n_static + 1)
                ]
                for i in range(1, interval):
                    self.k = i
                    self.compute_cir(init=False, T=t)
                    if self.l == 0.005:
                        up_rx_signal = self.get_rxsignal(plot=False)
                        self.sampling(up_rx_signal, plot=False)
                        h = self.estimate_CIR(self.rx_signal, plot=False)
                ...
                if self.v_rx == 0:
                    ...
                    eta, f_d, v = self.solve_system(phase_diff, AoA, T=t)
                    x0 = [f_d, v, eta]
                ...
                if self.n_static == 6:
                    for j in [2, 4, 6]:
                        results = least_squares(
                            self.system, x0, args=(phase_diff[: j + 2], AoA[: j + 2])
                        )
                        npath_err.append(
                            abs((self.fd - np.mean(results.x[0])) / self.fd)
                        )
                else:
                ...
                if relative:
                    if npath_err:
                        fd_err.append(npath_err)
                    else:
                        err = abs((self.fd - np.mean(results.x[0])) / self.fd)
                        fd_err.append(err)
                else:
                    fd_err.append(abs(self.fd - np.mean(results.x[0])))
            f_d_error.append(fd_err)
        return f_d_error
...
if __name__ == "__main__":
    ...
    ns = 6
    ch_sim = channel_sim(vmax=10, SNR=20, l=0.0107, n_static=ns)
    i = int(interval * 1e-3 / ch_sim.T)
    # npath_error = ch_sim.simulation(x_max=50,y_max=50,N=10000,interval=i)
    # npath_error = np.stack(npath_error,axis=0)
    # for i in range(3):
    #     print('average fd estimate relative error ns=%s : '%((i+1)*2) + str(np.mean(npath_error[:,:,i], axis=0))+'\n')
    #     print('median fd estimate relative error ns=%s : '%((i+1)*2) + str(np.median(npath_error[:,:,i],axis=0))+'\n')

    # np.save('cir_estimation_sim/data/varying_T/tot_fd_error_fc28.npy', npath_error)
    npath_error = np.load("cir_estimation_sim/data/varying_T/tot_fd_error_fc28.npy")
    legend = ["2 static paths", "4 static paths", "6 static paths"]
    times = np.round(np.arange(0.18, 0.72, 0.02), 2)
    # ch_sim.plot_mae(npath_error,'cir_estimation_sim/plot/T/',legend,times)
    errors = np.reshape(npath_error, -1, order="F")
    times = np.repeat(times, npath_error.shape[0])
    times = np.tile(times, npath_error.shape[2])
    npaths = np.repeat([2, 4, 6], npath_error.shape[0] * npath_error.shape[1])
    data = np.array([times, errors, npaths]).swapaxes(0, 1)
    df = pd.DataFrame(data=data, columns=["time", "error", "npaths"])
